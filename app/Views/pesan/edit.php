                        ...
                        <form class="form" action="/pesan/save" method="post" enctype="multipart/form-data">
                            ...
                            <div class=" my-3 p-0">
                                <select class="col-12 text-center form-select" name="tempat" id="tempat">
                                    <option value="<?= (old('tempat')) ? old('tempat') : $pesan['tempat'] ?>"><?= (old('tempat')) ? old('tempat') : $pesan['tempat'] ?></option>
                            ...
                            <div class=" my-3 p-0 d-flex">
                                <div class="form-group col-6">
                                    <input type="date" value="<?= (old('tanggal')) ? old('tanggal') : $pesan['tanggal'] ?>" class="form-control <?= ($validation->hasError('tanggal')) ? 'is-invalid' : ''; ?>" id="tanggal" name="tanggal">
                                </div>
                                <div class="invalid-feedback">
                                    <?= $validation->getError('tanggal'); ?>
                                </div>
                                <select class="text-center form-select" name="jam" id="jam">
                                    <option value="<?= (old('jam')) ? old('jam') : $pesan['jam'] ?>"><?= (old('jam')) ? old('jam') : $pesan['jam'] ?></option>
                            ...
                            <div class=" my-3 d-flex">
                                <div class="my-3 d-flex">
                                    <div class="col-6 d-flex border">
                                        ...
                                        <input class="form-control text-center <?= ($validation->hasError('jumlah')) ? 'is-invalid' : ''; ?>" type="text" id="jumlah" value="<?= (old('jumlah')) ? old('jumlah') : $pesan['jumlah'] ?>" name="jumlah">
                                        <button class="btn plus-btn" type="button" onclick="incrementJumlah()"><i class="far fa-plus-square"></i></button>
                                    </div>
                                    <div class="col-6 ">
                                        <input type="text" id="price" name="price" class="text-dark form-control" value="<?= (old('price')) ? old('price') : $pesan['price'] ?> " readonly>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="my-3 bangku  p-2 ">
                                <div class="seats p-0">
                                    <?php foreach ($seats as $s) : ?>
                                        <!-- <td></td> -->
                                        <input class="btn seat-btn" type="checkbox" name="seat[]" id="seat" value="<?= $s['nama_bangku']; ?>">
                                    <?php endforeach; ?>
                                </div>
                            ...
                            <div class=" d-flex justify-content-start p-0">
                                <button class="submit  px-4" type="submit"><i class="fas fa-pencil-alt"></i> UBAH PESANAN</button>
